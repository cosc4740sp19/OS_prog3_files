...
/*
 * a note for this version.  
 * instead of using a mutex to find the id number, it passes a parameter that is the unique id number for each thread.
 */
...
// create threads 
  for (i=0; i<num_c; i++) {
    cons[i] = thread(consum, consumern);
    consumern++;
  }
  printf ("consmer threads created\n");

  for (i=0; i<num_p; i++) {
    prod[i] = thread(produc, producern);
    producern++;
  }
